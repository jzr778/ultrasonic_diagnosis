...
在 `val_dataset_v5`（757 case）上对比微调模型（EAS 部署）与gemini-3.1-pro-preview（VLM 零样本）的误检诊断效果：
...
微调模型在各指标上均显著优于通用gemini-3.1-pro-preview，误检 Recall 提升约 20 个百分点，整体 Macro-F1 领先约 17 个百分点。