...
def extract_feishu_id(name):
    """从 name 字段中提取飞书 ID (例如：LPA05-120003819-xxx -> 120003819)"""
    if not name:
        return None
    # 匹配模式：第一个连字符后的数字串
...
def main():
    ...
    parser.add_argument("-o", "--output", default="tag_id_list.json", help="导出的 JSON 文件路径")
    ...
    if args.feishu_id_only:
        ...
        for d in all_details:
            name = d.get("name")
            feishu_id = extract_feishu_id(name)
            if feishu_id:
                feishu_ids.append(int(feishu_id))
        
        results = feishu_ids
        print(f"   -> 成功提取 {len(feishu_ids)} 个飞书 ID")
    else:
        # 输出完整数据
        results = []
        for d in all_details:
